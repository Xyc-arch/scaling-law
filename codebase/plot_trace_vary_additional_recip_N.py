#!/usr/bin/env python3

import os
import json
import matplotlib.pyplot as plt
import numpy as np
from sklearn.linear_model import LinearRegression
from utils import *

def plot_query_data_multiple(data_name, json_dir, task, classifier, metric):
    
    metric_names = {
        "balanced": "balanced accuracy", 
        "worst": "worst accuracy", 
        "min": "minority accuracy", 
        "balanced_log_cross": "balanced cross entropy loss", 
        "min_log_cross": "minority cross entropy loss"
    }
    metric_name = metric_names[metric]
        
    task_names = {
        "spurious_corr_vary_additional": "Spurious Corr Augment Additional", 
        "imbalanced_class_vary_additional": "Imbalanced Class Augment Additional"
    }
    task_name = task_names[task]
    
    clf_names = {"xgb": "XGBoost", "rf": "Random Forest"}
    clf_name = clf_names[classifier]
    
    colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k', 'orange', 'purple', 'pink']

    plt.figure(figsize=(10, 6))
    file_name = f"{task}_{classifier}_synthetic_allseed_{metric}.json"
    file_path = os.path.join(json_dir, file_name)
        
    if not os.path.exists(file_path):
        raise ValueError(f"File {file_name} not found in {json_dir}.")
    
    means = []
    stds = []

    with open(file_path, 'r') as f:
        data = json.load(f)

    # ------------------------------------------------
    # Load means/stds as stored (currently log-loss).
    # ------------------------------------------------
    for i in range(10):
        key = str(i)
        mean_key = "balance_augment_mean"
        std_key = "balance_augment_std"
        means.append(data[key][mean_key])
        stds.append(data[key][std_key])

    means = np.array(means)
    stds = np.array(stds)

    # ------------------------------------------------
    # N values (original spacing preserved)
    # [1, 401, 801, ..., 3601]
    # ------------------------------------------------
    N = np.arange(1, 4001, 400)

    # ------------------------------------------------
    # If metric is a log-cross-entropy variant:
    # - JSON values are mean/std of log(loss)
    # - Convert to mean/std of loss via log-normal formulas:
    #     loss_mean = exp(mu + 0.5 * sigma^2)
    #     loss_std  = sqrt( (exp(sigma^2) - 1) * exp(2mu + sigma^2) )
    # - x-axis uses 1/N (ignoring N=1).
    # ------------------------------------------------
    if metric in ["balanced_log_cross", "min_log_cross"]:
        mu = means
        sigma = stds

        loss_means = np.exp(mu + 0.5 * (sigma ** 2))
        loss_stds = np.sqrt((np.exp(sigma ** 2) - 1.0) * np.exp(2 * mu + (sigma ** 2)))

        means = loss_means
        stds = loss_stds

        traces = 1.0 / N
        x_plot = traces[1:]          # ignore first point (N=1)
        means_plot = means[1:]
        stds_plot = stds[1:]
    else:
        # Accuracy-style metrics: no transform; use N directly.
        traces = N
        x_plot = traces
        means_plot = means
        stds_plot = stds

    # ------------------------------------------------
    # Plot error bars (no connecting line)
    # ------------------------------------------------
    plt.errorbar(
        x_plot,
        means_plot,
        yerr=stds_plot,
        fmt='o',
        color=colors[0],
        capsize=5,
        label="augment additional",
        linestyle='None'
    )

    # ------------------------------------------------
    # Linear regression fit on plotted region
    # ------------------------------------------------
    x_fit = x_plot.reshape(-1, 1)
    y_fit = means_plot

    linear_model = LinearRegression()
    linear_model.fit(x_fit, y_fit)
    y_pred = linear_model.predict(x_fit)

    plt.plot(x_plot, y_pred, color='blue', label='fitted linear line')

    # ------------------------------------------------
    # Horizontal line: train_all_augment baseline
    # For log-cross metrics, assume stored as log(loss),
    # so convert with exp to get loss scale.
    # ------------------------------------------------
    if "10" in data and "train_all_augment" in data["10"]:
        base_val = data["10"]["train_all_augment"]
        if metric in ["balanced_log_cross", "min_log_cross"]:
            base_val = np.exp(base_val)
        plt.axhline(
            y=base_val,
            color='r',
            linestyle='--',
            label=f'train all real {base_val: .3f}'
        )

    # ------------------------------------------------
    # For loss-vs-1/N plots we DO NOT draw no-augmentation baseline.
    # For accuracy metrics, keep baseline at means[0].
    # ------------------------------------------------
    if metric not in ["balanced_log_cross", "min_log_cross"]:
        plt.axhline(
            y=means[0],
            color='g',
            linestyle=':',
            label=f'balanced mean without additional, {means[0]:.3f}'
        )

    # ------------------------------------------------
    # Axis labels + zoom
    # ------------------------------------------------
    if metric in ["balanced_log_cross", "min_log_cross"]:
        plt.xlabel('1 / # additional augmented synthetic samples', fontsize=14)
        x_min = 0.0
        x_max = x_plot.max() * 1.05
        plt.xlim(x_min, x_max)
    else:
        plt.xlabel('# additional augmented synthetic samples', fontsize=14)

    plt.ylabel(metric_name, fontsize=14)
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    plt.title(f'Performance for Task: {task_name}, Classifier: {clf_name}')
    plt.legend(fontsize=12)
    plt.grid(True)

    # ------------------------------------------------
    # Save
    # ------------------------------------------------
    output_dir = f"/home/ubuntu/plots/{data_name}"
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(f"{output_dir}/{task}_{classifier}_{metric}.png")
    plt.show()


# ===================================================================
# MASTER LOOP — unchanged
# ===================================================================
if __name__ == "__main__":
    
    data_names = {0: "craft", 1: "gender", 2: "diabetes", 3: "adult"}
    tasks = {0: "spurious_corr_vary_additional", 1: "imbalanced_class_vary_additional"}
    metrics = {0: "balanced_log_cross", 1: "min_log_cross"}
    classifier_types = {0: "rf", 1: "xgb"}
    
    for idx_data_name in [0, 1, 2, 3]:
        data_name = data_names[idx_data_name]
        for idx_task in [0, 1]:
            task = tasks[idx_task]
            if data_name == "adult" and task == "spurious_corr_vary_additional":
                continue
            for idx_metric in [0, 1]:
                metric = metrics[idx_metric]
                for idx_classifier in [0, 1]:
                    classifier = classifier_types[idx_classifier]
        
                    info = load_data_info('data_info.json')
                    data_info = info.get(data_name, {})
                    json_dir = data_info.get('path')
                    
                    plot_query_data_multiple(
                        data_name,
                        json_dir,
                        task,
                        classifier,
                        metric
                    )
