...
def split_and_save_syn(file_path, output_train1, output_train0, output_pos, output_neg, output_data_size, random_seed=42, label_column='Target', spurious_column='X6', spurious_small_is_0=1):
    """
    Split a synthetic dataset into label-based and spurious-correlation-based subsets.
    ...
    output_train1 : str
        Path where the subset with label equal to 1 is saved.
    output_train0 : str
        Path where the subset with label equal to 0 is saved.
    output_pos : str
    ...
    output_data_size : str
        Path where a JSON file containing output subset sizes is saved.
    random_seed : int, optional
        Random seed used when shuffling each output subset. Default is 42.
    ...
    -------
    None
        Writes the label-based subsets, spurious-correlation subsets, and subset size metadata to disk.
    """
