...
def load_data_info(json_file):
    """
    Load dataset configuration information from a JSON file.

    Parameters
    ----------
    json_file : str
        Path to the JSON file containing dataset configuration metadata.

    Returns
    -------
    dict
        Dictionary containing dataset-specific configuration information.
    """
...
def loss(metric, test_data, test_groups, target_column, clf, task):
    """
    Compute the evaluation loss for imbalanced-classification or spurious-correlation experiments.

    Parameters
    ----------
    metric : str
        Metric identifier. Supported values include "balanced_log_cross", "balanced_cross",
        "min_log_cross", and "min_cross".
    test_data : pandas.DataFrame
        Full test dataset containing features and the target column.
    test_groups : dict
        Dictionary of grouped test subsets used for minority-group or class-specific evaluation.
    target_column : str
        Name of the binary target column.
    clf : object
        Fitted classifier with ``predict`` and ``predict_proba`` methods.
    task : str
        Experiment type. Supported values are "imbalance" and "spurious".

    Returns
    -------
    float
        Computed loss value for the requested metric and task.
    """
...
def get_classifier(classifier_type, random_state):
    """
    Initialize a classifier by model type.

    Parameters
    ----------
    classifier_type : str
        Classifier identifier. Supported values are "log", "rf", "cat", "gb", and "xgb".
    random_state : int
        Random seed passed to the classifier constructor.

    Returns
    -------
    object
        Initialized scikit-learn, CatBoost, or XGBoost classifier.
    """
