...
def divide_test_data(test_data, spurious_column, spurious_small_is_0, label_column="Target"):
    ...
    test_data["spurious_value"] = test_data[spurious_column].apply(
...
def run_experiment(parent_path, seed_size, classifier_type,
                   spurious_column, spurious_small_is_0,
                   target_column, add_data, metric):
    """
    Run the independent-synthetic-data spurious-correlation ablation experiment.

    Parameters
    ----------
    parent_path : str
        Directory containing seed, training, independent synthetic, and test CSV files.
    seed_size : int
        Size of the seed dataset used to determine sampling sizes.
    classifier_type : str
    ...
    add_data : str
        Name of the independent synthetic augmentation method used to locate input files.
    metric : str
        Evaluation metric identifier passed to ``loss``.

    Returns
    -------
    dict
        Dictionary containing means and standard deviations for the spurious-imbalance baseline
        and the balanced-with-independent-synthetic-data condition.
    """
    ...
    test_groups = divide_test_data(
        test_data.copy(), spurious_column, spurious_small_is_0, target_column
    )
    ...
    for seed in [1, 2, 6, 8, 42]:
        ...
        n_samples = imbalance_ratio * seed_size

        # --- minimal change: allow replacement if too few rows ---
        train_neg_use = train_neg.sample(
            n=n_samples,
            random_state=seed,
            replace=(len(train_neg) < n_samples),
        )
        synthetic_pos_balance = synthetic_pos.sample(
            n=n_samples,
            random_state=seed,
            replace=(len(synthetic_pos) < n_samples),
        )
        # ---------------------------------------------------------
        ...
        imbalance_base_scores.append(
            loss(metric, test_data, test_groups, target_column, clf, "spurious")
        )
...
if __name__ == "__main__":
    ...
    # use only IND synthetic
    add_data_ls = {0: "synthetic_seed_ind"}
    classifier_types = {0: "rf", 1: "xgb"}
    metrics = {0: "balanced_cross", 1: "min_cross"}

    # run all four datasets (change if you only want a subset)
    for idx_data_name in [0, 1, 2, 3]:
        ...
        for idx_add_data in [0]:  # only synthetic_seed_ind
        ...
        save_path = os.path.join(RESULTS_DIR, f"{data_name}_ablate_spurious_ind.json")
