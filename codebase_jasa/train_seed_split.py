...
def train_seed_split(file_path, seed_size, output_seed, output_train, output_train1, output_train0, random_seed=42, label_column='label'):
    """
    Split a dataset into seed and training subsets, then split the training subset by label.
    ...
    seed_size : int
        Number of rows assigned to the seed dataset.
    output_seed : str
        Path where the seed dataset CSV file is saved.
    output_train : str
        Path where the remaining training dataset CSV file is saved.
    output_train1 : str
        Path where the label-1 training subset CSV file is saved.
    output_train0 : str
        Path where the label-0 training subset CSV file is saved.
    random_seed : int, optional
        Random seed used when shuffling rows before splitting. Default is 42.
    label_column : str, optional
        Name of the binary label column used to split the training subset. Default is 'label'.

    Returns
    -------
    None
        Writes the seed, training, label-1 training, and label-0 training CSV files to disk.
    """
...
def train_seed_split_balanced(file_path, seed_size, output_seed, output_train, output_train1, output_train0, output_pos, output_neg, output_data_size, random_seed=42, label_column='Target', spurious_column='X6', spurious_small_is_0=1):
    """
    Create a balanced seed set across label-spurious groups and save disjoint training subsets.
    ...
    file_path : str
        Path to the input CSV file.
    seed_size : int
        Total number of rows assigned to the balanced seed dataset.
    output_seed : str
        Path where the balanced seed dataset CSV file is saved.
    output_train : str
        Path where the remaining training dataset CSV file is saved.
    output_train1 : str
        Path where the training subset with label equal to 1 is saved.
    output_train0 : str
        Path where the training subset with label equal to 0 is saved.
    output_pos : str
        Path where the positive spurious-correlation subset is saved.
    output_neg : str
        Path where the negative spurious-correlation subset is saved.
    output_data_size : str
        Path where a JSON file containing output dataset sizes is saved.
    random_seed : int, optional
        Random seed used for sampling and shuffling. Default is 42.
    label_column : str, optional
        Name of the binary label column. Default is 'Target'.
    spurious_column : str, optional
        Name of the spurious attribute column. Default is 'X6'.
    spurious_small_is_0 : int, optional
        Indicator for how to define the smaller spurious value. If 1, the smaller value is set to 0;
        otherwise, the minimum observed value of the spurious column is used. Default is 1.

    Returns
    -------
    None
        Writes the balanced seed set, training set, training subsets, spurious-correlation subsets,
        and dataset size metadata to disk.
    """
